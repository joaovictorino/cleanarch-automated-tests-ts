...
function ${IS_MUTANT_ACTIVE_HELPER}(id) {
  ...
  return isActive(id);
}`, { configFile: false })).program.body; // cast here, otherwise the thing gets unwieldy to handle
/**
...
export function mutationCoverageSequenceExpression(mutants, targetExpression) {
    ...
    const sequence = [types.callExpression(types.identifier(COVER_MUTANT_HELPER), mutantIds)];
    if (targetExpression) {
