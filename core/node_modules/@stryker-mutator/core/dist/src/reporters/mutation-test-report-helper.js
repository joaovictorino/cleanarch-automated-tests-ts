...
export class MutationTestReportHelper {
    constructor(reporter, options, project, log, testCoverage, fs, requireFromCwd) {
    ...
    async mutationTestReport(results) {
        ...
        const remapTestId = (id) => { var _a; return (_a = testIdMap.get(id)) !== null && _a !== void 0 ? _a : id; };
        const remapTestIds = (ids) => ids === null || ids === void 0 ? void 0 : ids.map(remapTestId);
        return {
    ...
    async toFileResults(results, remapTestIds) {
        ...
        return results.reduce((acc, mutantResult) => {
            ...
            const fileResult = (_a = acc[reportFileName]) !== null && _a !== void 0 ? _a : (acc[reportFileName] = fileResultsByName.get(mutantResult.fileName));
    ...
    async toTestFiles(remapTestId) {
        ...
        return [...this.testCoverage.testsById.values()].reduce((acc, testResult) => {
            ...
            const testFile = (_a = acc[reportFileName]) !== null && _a !== void 0 ? _a : (acc[reportFileName] = testFilesByName.get(reportFileName));
    ...
    async toFileResult(fileName) {
        ...
        if (sourceFile) {
            ...
            this.log.warn(normalizeWhitespaces(`File "${fileName}" not found
    ...
    async toTestFile(fileName) {
        ...
        if (fileName) {
            ...
            if (file) {
                ...
                this.log.warn(normalizeWhitespaces(`Test file "${fileName}" not found
    ...
    discoverDependencies() {
        const discover = (specifier) => {
            try {
                return [specifier, this.requireFromCwd(`${specifier}/package.json`).version];
            }
            catch {
        ...
        return dependencies.map(discover).reduce((acc, dependency) => {
            if (dependency) {
...
MutationTestReportHelper.inject = tokens(coreTokens.reporter, commonTokens.options, coreTokens.project, commonTokens.logger, coreTokens.testCoverage, coreTokens.fs, coreTokens.requireFromCwd);
