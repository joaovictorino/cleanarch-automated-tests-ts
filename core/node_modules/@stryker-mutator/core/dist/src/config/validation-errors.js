 ...
 */
export function describeErrors(allErrors) {
 ...
 *  ```
 */
function filterRelevantErrors(allErrors) {
...
function removeShadowingErrors(singleErrors, metaErrors) {
    return singleErrors.filter((error) => {
        if (metaErrors.some((metaError) => error.instancePath.startsWith(metaError.instancePath))) {
            return !singleErrors.some((otherError) => otherError.instancePath.startsWith(error.instancePath) && otherError.instancePath.length > error.instancePath.length);
        }
        else {
            return true;
        }
...
function describeError(error) {
    ...
    switch (error.keyword) {
        ...
        case 'enum':
            return `${errorPrefix} should be one of the allowed values (${error.params.allowedValues.map(stringify).join(', ')}), but was ${stringify(error.data)}.`;
        case 'minimum':
        case 'maximum':
