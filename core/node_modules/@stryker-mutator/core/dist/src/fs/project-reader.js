...
export class ProjectReader {
    constructor(fs, log, { mutate, tempDirName, ignorePatterns, incremental, incrementalFile, force, htmlReporter, jsonReporter }) {
        this.fs = fs;
        this.log = log;
     ...
     */
    resolveFileDescriptions(inputFileNames) {
        ...
        const mutateInputFileMap = new Map();
    ...
    mergeFileDescriptions(first, second) {
        if (second) {
    ...
    async readIncrementalReport() {
        ...
        try {
            ...
            return {
                ...result,
                files: Object.fromEntries(Object.entries(result.files).map(([fileName, file]) => [
                    fileName,
                    { ...file, mutants: file.mutants.map((mutant) => ({ ...mutant, location: reportLocationToStrykerLocation(mutant.location) })) },
                ])),
                testFiles: result.testFiles &&
                    Object.fromEntries(Object.entries(result.testFiles).map(([fileName, file]) => [
                        fileName,
                        {
                            ...file,
...
ProjectReader.inject = tokens(coreTokens.fs, commonTokens.logger, commonTokens.options);
...
//# sourceMappingURL=project-reader.js.map