...
export class StrykerInitializer {
    ...
    async initiateCustom(configWriter) {
        ...
        const pkgInfoOfSelectedTestRunner = packageInfo.find((pkg) => { var _a; return pkg.name == ((_a = selectedTestRunner.pkg) === null || _a === void 0 ? void 0 : _a.name); });
        const additionalConfig = packageInfo.map((dep) => { var _a; return (_a = dep.initStrykerConfig) !== null && _a !== void 0 ? _a : {}; }).filter(notEmpty);
        const configFileName = await configWriter.write(selectedTestRunner, buildCommand, selectedReporters, selectedPackageManager, npmDependencies.map((pkg) => pkg.name), additionalConfig, (_a = pkgInfoOfSelectedTestRunner === null || pkgInfoOfSelectedTestRunner === void 0 ? void 0 : pkgInfoOfSelectedTestRunner.homepage) !== null && _a !== void 0 ? _a : "(missing 'homepage' URL in package.json)", isJsonSelected);
...
StrykerInitializer.inject = tokens(commonTokens.logger, initializerTokens.out, initializerTokens.npmClient, initializerTokens.customInitializers, initializerTokens.configWriter, initializerTokens.gitignoreWriter, initializerTokens.inquirer);
